...
Quintus["2D"] = function(Q) {
 ...
 Q.Sprite.extend("TileLayer",{
    ...
    init: function(props) {
      ...
      if(this.p.dataAsset) {
        if(this.p.width){
          this.ParseLDKCollisionMap(this.p.dataAsset,this.p.width)
        }else{
    ...
    ParseLDKCollisionMap:function(inputString,mapWidth){
      console.log(inputString)
      const inputArray = inputString.split(',').map(Number);
      const mapHeight = Math.ceil(inputArray.length / mapWidth);
      const collisionMap = [];
    
      for (let y = 0; y < mapHeight; y++) {
        const startIndex = y * mapWidth;
        const endIndex = Math.min(startIndex + mapWidth, inputArray.length);
        const row = inputArray.slice(startIndex, endIndex);
        collisionMap.push(row);
      }
    
      this.p.tiles=collisionMap;
